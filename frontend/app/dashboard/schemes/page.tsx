    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        const mapped: SchemeVerdict[] = (data ?? []).map((row: any) => ({
          scheme_name: row.form_data?.scheme_name ?? row.scheme_name,
          level: row.form_data?.level ?? "state",
          category: row.form_data?.category ?? "",
          reason: row.form_data?.reason ?? "",
          confidence: row.form_data?.confidence ?? "medium",
          eligible: row.form_data?.eligible ?? true,
          missing_info: row.form_data?.missing_info ?? [],
          is_automatable: row.form_data?.is_automatable ?? false,
        }));
    ...
    <section className="flex flex-col gap-4">
      ...
      {!loading && schemes.length === 0 && (
        <div className="bg-white rounded-2xl border border-dashed border-slate-300 px-5 py-10 text-center">
          <p className="text-sm text-slate-400">No eligible schemes found yet.</p>
        </div>
      )}
